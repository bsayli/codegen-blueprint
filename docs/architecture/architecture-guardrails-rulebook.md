...
* [Appendix A — Architecture Contract Vocabulary](#appendix-a--architecture-contract-vocabulary)
* [Summary](#summary)
...
1. **Dependency guardrails** — control allowed dependency directions
2. **Boundary guardrails** — protect public and framework-facing boundaries
3. **Schema & contract guardrails** — protect the *meaning* and *scope* of the architecture itself
...
Hexagonal Strict enforces **full architectural contract integrity** across adapters, application, and domain — and adds **local cycle prevention** plus **REST boundary isolation**.
...
> Note: This is **family-local** cycle prevention (not “between top-level packages”).
...
* No strict controller → service → repository call ordering
...
These are deliberately deferred to **Standard — Strict** to keep adoption friction low.
...
It goes beyond “don’t do obvious mistakes” and instead validates **layer purity**, **domain purity**, **cycle-freedom**
...
This prevents “domain as a dumping ground” and ensures the domain remains stable and portable.
...
* DTOs stay boundary-local and do not “import the domain” by accident.
...
It turns “layered architecture” from a convention into an **executable contract**.
...
## Appendix A — Architecture Contract Vocabulary
...
> **It fails — clearly, early, and on purpose.**
