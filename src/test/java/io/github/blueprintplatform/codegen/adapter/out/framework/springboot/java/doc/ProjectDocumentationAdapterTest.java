...
class ProjectDocumentationAdapterTest {
  ...
  void generate_shouldBuildCorrectModelForProjectDocumentation() {
    ...
    List<BuildDependency> mappedDeps =
        List.of(BuildDependency.of("org.acme", "custom-dep", "1.0.0", "runtime"));
    RecordingBuildDependencyMapper mapper = new RecordingBuildDependencyMapper(mappedDeps);
    ...
    List<BuildDependency> deps =
        (List<BuildDependency>) model.get(ProjectDocumentationModel.DEPENDENCIES);
    ...
    BuildDependency d = deps.getFirst();
